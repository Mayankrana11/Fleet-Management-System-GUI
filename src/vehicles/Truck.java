...
public class Truck extends LandVehicle implements FuelConsumable, CargoCarrier, Maintainable {
    ...
    public void move(double distance) throws InvalidOperationException, InsufficientFuelException {
        ...
        if (fuelLevel + FUEL_EPS < requiredFuel) {
            throw new InsufficientFuelException("Not enough fuel, please refuel.");
        }
        fuelLevel -= requiredFuel;
        if (fuelLevel < FUEL_EPS) fuelLevel = 0.0;
        addMileage(distance);
    ...
    public double consumeFuel(double distance) throws InsufficientFuelException {
        ...
        if (fuelLevel + FUEL_EPS < required) {
            throw new InsufficientFuelException("Not enough fuel, please refuel.");
        }
        fuelLevel -= required;
        if (fuelLevel < FUEL_EPS) fuelLevel = 0.0;
    ...
    public boolean needsMaintenance() {
        return maintenanceNeeded || (getCurrentMileage() - getLastMaintenanceMileage()) > 15000;
    }
